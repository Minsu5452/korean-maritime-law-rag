...
LLM은 질문 분류, 근거 평가, 쿼리 재작성, 답변 생성에 사용하고, 수집·파싱·검색·인용 검증은 가능한 한 LLM 없이 재현 가능한 코드 경로로 처리합니다.
...
조문과 별표를 같은 검색 단위로 다루고, 법령명, 조문 번호, 제목, 시행일, 법령 종류, 상위 법률 ID를 메타데이터로 보관합니다.
...
- `VectorIndex`: Qdrant에 임베딩 벡터 저장, `doc_id` 기반 UUID5로 멱등 업서트
...
이 확장 후보는 별도 순위 목록으로 다뤄 RRF에 합산합니다.
질문에 `「법령명」 제N조` 형태가 들어 있으면 `parse_citation()`으로 먼저 추출해 graph 검색에서 해당 조문을 최상위로 고정합니다. 이때 현재 최고 RRF 점수보다 1.0 높은 점수를 부여합니다. 로컬 인덱스에 그 조문이 없고 실시간 조회가 켜져 있으면 law.go.kr를 한 번 조회해 보강합니다.
...
`/query/stream`은 LangGraph 노드 진행을 SSE로 내보냅니다.
...
Langfuse는 선택 기능이며, 키나 패키지가 없으면 모니터링 없이 실행됩니다.
...
질문에 지목된 조문이 로컬 인덱스에 없을 때 law.go.kr를 질의 시점에 조회하는 실시간 조회도 선택 기능입니다.
...
- `MLR_AS_OF=YYYYMMDD`는 ablation·정답 체인 추출 스크립트에서 해당 일자 기준 현행 조문만 남기는 필터입니다(`corpus.filter_as_of`). 기본 색인·서빙 경로는 캐시 전체를 사용하며, 과거 시점 전문 복원 기능은 아닙니다.
...
- 내부 평가용 골드셋이므로 외부 공개 벤치마크와는 범위가 달라 직접 비교하기 어렵습니다.
