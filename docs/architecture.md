...
질문에 `「법령명」 제N조` 형태가 들어 있으면 `parse_citation()`으로 먼저 잡아 graph 검색에서 해당 조문을 pinning합니다.
...
- `MLR_AS_OF=YYYYMMDD`는 현재 캐시에 들어 있는 조문 중 해당 일자에 시행 중인 조문만 사용하도록 필터링합니다. 과거 전문 복원 기능은 아닙니다.
