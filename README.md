...
기본 모드는 실제 실행을 녹화한 데모를 키 없이 재생합니다. 라이브 모드는 위 `uvicorn` 백엔드를 띄운 뒤 화면 우상단에서 전환하면 로컬 백엔드에 직접 질의합니다. 데모에 쓰는 질의·응답은 `scripts/capture_demo_traces.py`로 다시 생성할 수 있습니다.
...
- 그래프 검색은 단독 전략이라기보다 정답 후보를 넓히는 용도로 썼습니다. 현재 평가에서 그래프의 hit@1(0.279)은 벡터 검색(0.539)보다 낮지만 recall@10(0.945)은 세 전략 중 가장 높았습니다. Neo4j 의존성을 떠안는 대신, 질문에 지목된 조문을 정확히 고정하고 법률→시행령→시행규칙 위임(IMPLEMENTS) 체인을 따라가며 정답 후보를 가장 넓게 확보할 수 있습니다. 이 비용이 부담되면 Neo4j 없이 벡터나 하이브리드 전략만으로도 검색은 동작합니다.
