...
해양경찰청 현장 실무를 가정해 한국 정부 디자인 시스템(KRDS) 기준으로 만든 [웹 데모](web/)입니다. 실제 실행을 녹화해 키 없이 재생하며, 로컬에서는 라이브 모드로 백엔드에 직접 질의할 수 있습니다.
...
- 해양경찰청 현장 실무를 가정한 Next.js 웹 데모(정부 디자인 시스템 KRDS 기준): 답변 근거 도출 과정 시각화, 근거 조문·시행일·원문 링크, 녹화/라이브 모드
...
답변 생성까지 포함한 전체 응답 시간과 질의당 비용입니다. 에이전트는 `gpt-4o-mini`로 동작하고, 생성 품질을 평가하는 judge 모델만 `gpt-4o`로 분리했습니다.
