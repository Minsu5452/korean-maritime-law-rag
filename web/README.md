...
상위 `korean-maritime-law-rag` 백엔드의 검색·생성 과정을 단계별로 보여주는 Next.js 프론트엔드입니다.
에이전트가 질문을 분류하고, 검색하고, 근거를 평가하고, 답변을 생성한 뒤 인용을 검증하는 흐름을
그대로 시각화하고, 답변이 근거로 삼은 조문을 카드로 보여줍니다.
